...
import LogoFaceBook from '~/components/svg/facebook.svg?react';
import LogoInstagram from '~/components/svg/instagram.svg?react';
import LogoTossBlog from '~/components/svg/toss-blog.svg?react';
import LogoTossNaverPost from '~/components/svg/toss-naver-post.svg?react';
import LogoTwitter from '~/components/svg/twitter.svg?react';
...
const FooterNav = ({ title, data }: FooterNavProps) => {
  return (
    <nav className="min-w-[150px]">
      <h3 className="pb-[10px] text-[15px] font-bold text-[#333d4b]">{title}</h3>
      <ul>
        {data.map((item, index) => (
          <li
            key={index}
            className="pb-[5px] text-[15px] text-[#6b7684] hover:text-[#0056b3] hover:underline"
          >
...
export default function Footer() {
  return (
    <footer className="h-[917px] pt-[50px] pb-[100px]">
      <div className="m-auto max-w-[1064px] px-[67px]">
        ...
        <div>
          <div>
            <p className="pb-[16px] text-[15px] font-bold text-[#333d4b]">
              ㈜비바리퍼블리카
            </p>
            <p className="text-[13px] leading-[20px] text-[#8b95a1]">
              사업자 등록번호 : 120-88-01280 | 대표 : 이승건
              <br />
              호스팅 서비스 : 주식회사 비바리퍼블리카 | 통신판매업 신고번호 :
              2014-서울강남-03377{' '}
              <Link to="#" className="hover:text-[#0056b3] hover:underline">
                사업자정보확인
              </Link>
              <br />
              ...
              <ul key={i}>
                {terms.map((term, j) => (
                  <li
                    key={j}
                    className={cn(
                      'text-[13px] leading-[19.5px] font-[400] text-[#6b7684] hover:text-[#0056b3] hover:underline',
                      term.isBold && 'font-bold hover:text-[#6b7684] hover:underline',
                    )}
                  >
          ...
          <div className="flex gap-2">
            <Link to="#" className="opacity-70 duration-150 hover:opacity-100">
              <LogoFaceBook />
            </Link>
            <Link to="#" className="opacity-70 duration-150 hover:opacity-100">
              <LogoTossBlog />
